...
function Bro(bundleFile) {
  ...
  function createBundle(config) {
    ...
    // FIXME(nikku): OMG WTF
    //
    // expose all will change the semantics of how browserify
    // exports modules. With this option set to true we can require source files
    // by their absolute path names and still refer to them in test cases / stubs
    // using their relative names
    if (browserifyOptions.exposeAll === false) {
      log.warn('Overriding exposeAll browserify option with exposeAll=true.');
    }
    ...
    // perform additional setup via prebundle callback
